...
function loadTemplateReadme() {
	return fs.readFile(path.resolve(__dirname, './README.template.md'), 'utf8');
}

/**
 * @returns {CodepenFeedData}
 */
function getCodepenFeed() {
	// Starts at 1
	const page = 1;
...
function codepenFeedToTable(feed) {
	...
	const itemsData = items.map(n => {
		...
		const src = `https://codepen.io/${username}/pen/${penID}/image/default.png`;
